...
uoff32_t KVDAT_get_entry_key_by_off(
    FILE       *fp_dat,
    uoff32_t    entryoff,
    size32_t   *out_key_len_p,
    byte_t     *out_key_data_p
);
...
uoff32_t KVDAT_get_entry_key_by_off(
    ...
    if (is_null(fp) || is_null(out_key_len_p) || is_null(out_key_data_p))
        return 0;   // Err
    fseek(fp, entry_key_len_off(entryoff), SEEK_SET);
    size_t freadcnt = fread(out_key_len_p, sizeof(*out_key_len_p), 1, fp);
    if (freadcnt != 1) {
        printerrf("fread out_key_len_p failed\n");
        return 0;
    }
    out_key_data_p = (byte_t*)malloc(*out_key_len_p);
    if (!out_key_data_p) {
        printerrf("malloc out_key_data_p failed");
        return 0;
    }
    freadcnt = fread(out_key_data_p, 1, *out_key_len_p, fp);
    if ((size32_t)freadcnt != *out_key_len_p) {
...
static inline FILE *KVDAT_open_datfile(const char *filepath, char *mode) {
    if (is_null(filepath) || is_null(mode)) return NULL;

    FILE *fp_dat = fopen(filepath, mode);
    if (!fp_dat) {
        printerrf("fopen fp_dat failed with %s: %s\n", mode, strerror(errno));
        return NULL;
    }
    return fp_dat;
}
