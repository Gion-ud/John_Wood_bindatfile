...
void DAT_FILE_FOOTER_fill(
    ...
    *f_ftr_p = (DAT_FILE_FOOTER) {
        .crc32 = compute_mem_crc32(buf, footeroff),
        .magic = *(dword_t*)eof_magic_bytes
    };
}

static inline uoff32_t get_datasectionoff() {
    return PAD_OFF_ALIGN(sizeof(DAT_FILE_HEADER), PAD_SIZE);
}
static inline uoff32_t get_footeroff(word_t entrycount, size32_t entrysize) {
    return
        get_datasectionoff() +
        PAD_OFF_ALIGN(entrycount * entrysize, PAD_SIZE);
}

static inline uoff32_t get_filesize(word_t entrycount, size32_t entrysize) {
    return
        get_footeroff(entrycount, entrysize) + sizeof(DAT_FILE_FOOTER);
}
